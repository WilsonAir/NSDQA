...
from data.prediction_tools import to_predictions
...
def calcuber_single(pre, gt, image_name=None):
    ...
    return shadow, nonshadow, ber, S_f
...
def measure_bers(input, output, detection_results, mask, mask_size, selection_result):
    ...
    for idx in range(output.shape[0]):
        method_number_ith = F.softmax(output[idx], dim=0).argmax(dim=0).cpu()
        pre_ori = np.asarray(detection_results[idx][method_number_ith].cpu())
        ...
        gt = np.asarray(mask[idx].cpu()).squeeze(2)

        # pre_ori = cv.resize(pre_ori, (int(mask_size[idx][1]), int(mask_size[idx][0])))
        # gt = cv.resize(gt, (int(mask_size[idx][1]), int(mask_size[idx][0])))

        selection_result = calcuber_with_dict(pre_ori, gt, selection_result)
...
def measure_bers_for_ori(detection_results, mask, mask_size, detection_eval):
    ...
    for idx in range(detection_results.shape[0]):
        ...
        for idx2 in range(len(to_predictions)):
            ...
            pre = np.asarray(detection_results[idx][idx2].cpu())
            # pre = cv.resize(pre, (int(mask_size[idx][1]), int(mask_size[idx][0])))
